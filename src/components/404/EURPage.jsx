        ...
        {isLoading && (
          ...
          >
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="font-medium text-slate-500 dark:text-gray-400 animate-pulse">EUR ma'lumotlari yuklanmoqda...</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <div className="max-w-6xl mx-auto pt-[100px]">
            ...
            <div className="flex flex-col items-center justify-between gap-6 mb-10 md:flex-row" data-aos="fade-down">
              <div>
                ...
                <p className="mt-2 text-slate-500 dark:text-gray-400">{t("eurPage.subtitle")}</p>
              </div>
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
              ...
              <div className="lg:col-span-1 bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] shadow-xl shadow-indigo-100/20 dark:shadow-none border border-indigo-50 dark:border-gray-800 h-fit" data-aos="fade-right">
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block mb-3 ml-1 text-xs font-bold uppercase text-slate-400 dark:text-gray-500">{t("eurPage.enterAmount")}</label>
                    <div className="relative">
                      <input
                        ...
                        placeholder="0"
                        className="w-full p-5 pr-16 text-2xl font-black transition-all border-2 border-transparent outline-none bg-slate-50 dark:bg-gray-800 focus:border-indigo-500 focus:bg-white dark:focus:bg-gray-800 rounded-2xl dark:text-white"
                      />
                      <span className="absolute font-bold tracking-tighter -translate-y-1/2 pointer-events-none right-5 top-1/2 text-slate-400">EUR</span>
                    </div>
                  </div>

                  <div className="p-5 border border-indigo-100 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl dark:border-indigo-800/30">
                    <p className="text-xs font-bold tracking-wider text-indigo-600 uppercase dark:text-indigo-400">{t("eurPage.youGet")}</p>
                    <p className="mt-2 text-3xl font-black leading-tight text-indigo-800 break-words dark:text-indigo-300">{(amount * buyRate).toLocaleString()} <span className="text-sm font-medium tracking-normal opacity-70">UZS</span></p>
                  </div>

                  <button
                    onClick={handleExchange}
                    className="w-full py-5 text-sm font-black tracking-widest text-white uppercase transition-all bg-indigo-600 shadow-lg hover:bg-indigo-700 rounded-2xl active:scale-95 shadow-indigo-200 dark:shadow-none"
                  >
                    {t("eurPage.exchange")}
                  </button>
                </div>
              ...
              {isModalOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center w-16 h-16 mb-6 text-indigo-600 bg-indigo-100 dark:bg-indigo-900/30 rounded-2xl">
                      <CheckCircle2 size={32} />
                    </div>

                    <h2 className="mb-2 text-2xl font-black dark:text-white">{t("eurPage.modalTitle")}</h2>
                    <p className="mb-6 text-sm text-slate-500 dark:text-gray-400">{t("eurPage.modalText")}</p>
                    ...
                    <div className="flex flex-col gap-3">
                      <button
                        className="w-full py-4 font-bold text-white transition-all bg-indigo-600 rounded-2xl hover:bg-indigo-700"
                        onClick={() => setIsModalOpen(false)}
                      >
                        {t("eurPage.confirm")}
                      </button>
