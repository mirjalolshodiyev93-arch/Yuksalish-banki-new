      ...
      <div className="container relative px-6 mx-auto max-w-7xl">
        ...
        <div className="relative">
          ...
          {Array.isArray(timelineData) && timelineData.map((item, index) => (
            <div
              key={index}
              className={`relative flex items-center justify-between mb-32 w-full ${index % 2 === 0 ? 'md:flex-row-reverse' : 'md:flex-row'
                }`}
            >
              ...
              >
                <div className="flex items-center justify-center w-12 h-12 transition-transform bg-white border-4 border-green-500 shadow-xl dark:bg-slate-900 dark:border-green-400 rounded-xl group-hover:scale-110">
              ...
              <motion.div
                ...
                viewport={{ once: true }}
                className="w-full pl-16 md:w-5/12 md:pl-0"
              >
                <div
                  className={`
                  ...
                  `}
                >
                  <div className="absolute overflow-hidden pointer-events-none top-4 right-8">
                    <span className={`text-8xl font-black opacity-[0.03] dark:opacity-[0.05] italic ${index % 2 === 0 ? 'text-slate-900 dark:text-green-400' : 'text-white'
                      }`}>
                      {item.year}
                    </span>
                  </div>
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-4 mb-6">
                      <div className={`flex items-center justify-center w-10 h-10 rounded-2xl font-black text-sm shadow-inner ${index % 2 === 0
                          ? 'bg-green-50 text-green-600 dark:bg-green-900/30'
                          : 'bg-white/20 text-white'
                        }`}>
                        {index + 1}
                      </div>
                      <h3 className={`text-2xl font-black tracking-tight ${index % 2 === 0 ? 'text-slate-900 dark:text-white' : 'text-white'
                        }`}>
                        {item.title}
                      </h3>
                    </div>

                    <p className={`text-lg leading-relaxed font-medium ${index % 2 === 0 ? 'text-slate-600 dark:text-slate-400' : 'text-green-50/90'
                      }`}>
                      {item.desc}
                    </p>
                    ...
                    <div className="flex items-center gap-3 mt-8">
                      <div className={`h-1.5 w-12 rounded-full ${index % 2 === 0 ? 'bg-green-500' : 'bg-white'
                        }`} />
                      <div className={`w-1.5 h-1.5 rounded-full opacity-40 ${index % 2 === 0 ? 'bg-green-500' : 'bg-white'
                        }`} />
                    </div>
                  </div>
                </div>
